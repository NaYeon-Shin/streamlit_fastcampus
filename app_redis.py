...
from langchain_upstage import ChatUpstage # model
...
init_conversation() # 대화 초기화
print_conversation() # 지금 나누는거 말고 위에 대화 표시해주는 함수
...
if user_input:= st.chat_input("텍스트를 입력하세요."):
    ...
    with st.chat_message("assistant"):
        # 실시간으로 보여지는거 이전엔 커서 직접 구현했었는데,
        # StreamHandler로 구현 가능
        ...
        prompt = ChatPromptTemplate.from_messages(
            [
                ...
                # 여기서 history에 넣는건 이번에 나누는 대화
                # history랑 밑에 3번에서의 history 이름 동일해야해
